...
export default function VoiceAssistantCopilot({
  ...
  const handleCommand = async (rawText: string) => {
    ...
    try {
      ...
      const userId = localStorage.getItem('wa_userId') || 'user_1780714671963_281';
    ...
    >
      ...
      <div 
        ...
        style={{
          ...
          background: orbState === 'listening' ? 'var(--secondary)' : 'var(--primary)',
      ...
      >
        {orbState === 'listening' ? (
          ...
          <div className="spinner-border" style={{ width: '18px', height: '18px', border: '2px solid transparent', borderRightColor: '#fff', borderRadius: '50%', animation: 'spinner-border .5s linear infinite' }}></div>
        ) : (
          <span style={{ fontSize: '1.1rem' }}>⚡</span>
        )}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem', flex: 1, minWidth: '180px' }}>
        <span style={{ fontSize: '0.62rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', color: orbState === 'listening' ? 'var(--secondary)' : 'var(--primary)' }}>
          GiGO Siri Voice Copilot
        </span>
        <span style={{ fontSize: '0.75rem', color: 'var(--text-primary)', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {statusText}
        </span>
        {transcript && (
      ...
      {/* CSS Visualizer wave (rendered when active) */}
