...
To incentivize them, GiGO offers a free 250 Pace (Career Momentum Ledger) promotional signup bonus in their regional wallet (representing 160+ Autopilot Hours of saved labor) which can get them to compile their assets and send job applications immediately.
When they complete signup, the referrer "${referrerName}" will receive a 250 Pace referral bonus.
...
And highlight the 250 Pace promotional signup reward. Do NOT mention Naira, NGN, USD, cash, or "Tokens" in the generated message; use exclusively Pace (or Paces).
...
You MUST respond ONLY with a JSON object in this exact schema:
{
  ...
  "whatsappMessage": "A short, punchy WhatsApp message with emojis, the signup link, and highlight of the signup reward."
    ...
    let emailBody = `Hi ${friendName},\n\nYour colleague **${referrerName}** is inviting you to join **GiGO**! It is a state-of-the-art AI-Native Career Ecosystem designed to automate your job-seeking journey.\n\nRegister now and receive an instant **250 Pace Sign-up Reward** in your career wallet to compile ATS resumes and send job applications completely free: ${referralLink}`;
    let whatsappMessage = `Hey ${friendName}! 🚀 ${referrerName} referred you to join GiGO - an AI-powered job search platform! Get an instant 250 Pace Signup Reward to start applying: ${referralLink}`;
