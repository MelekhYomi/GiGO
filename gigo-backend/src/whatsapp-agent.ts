...
export async function processRealTimeJobMatchingNotifications(newJob: any) {
  ...
  for (const doc of usersSnapshot.docs) {
    ...
    if (matchScore >= 75) {
      const msg = `Hello ${userData.fullName}! 🚀 Match found: *${newJob.jobTitle}* at *${newJob.companyName}* (${matchScore}% alignment). Apply instantly: https://gigo-app-demo-xyz.web.app/dashboard`;
