  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [
        ...
        {
          text: `You are the core operational onboarding agent for GiGO. 
    ...
    if (response.text) {
      ...
      // 6. Trigger the Autonomous Job Scraper & Application pipeline in the background immediately
      console.log(`[AUTOPILOT] Triggering background job scraper and application engine for candidate ${userId}...`);
      executeAutonomousScraperPipeline(userId).catch(err => {
        console.error(`[AUTOPILOT ERROR] Failed to run deferred background scraper for candidate ${userId}:`, err);
      });
